...
class CustomCreateAvansnaFaktura extends Page implements HasForms
{
    ...
    protected static ?string $navigationLabel = 'Nova Avansna Faktura';

    protected static ?string $title = 'Nova Avansna Faktura';
    ...
    public function form(Schema $schema): Schema
    {
        return $schema
            ->schema([
                Section::make('Tip avansne fakture')
                    ->description('Izaberite tip avansne fakture na osnovu lokacije klijenta')
                    ->schema([
                        Radio::make('invoice_type')
                            ->label('Tip avansne fakture')
                            ->options([
                                'domestic' => 'Domaća avansna faktura',
                                'foreign' => 'Inostrana avansna faktura',
                            ])
                            ->default('domestic')
                            ->live()
                ...
                Section::make('Osnovne informacije')
                    ->schema([
                        ...
                        TextInput::make('invoice_number')
                            ...
                            ->placeholder('Ostavite prazno za automatsko generisanje (A1/2025)')
                            ->helperText('Ako ostavite prazno, broj će biti automatski generisan sa prefiksom A'),
                ...
                Section::make('Stavke avansne fakture')
                    ...
                    ->schema([
                        ...
                        Placeholder::make('invoice_total')
                            ->label('UKUPNO ZA AVANSNU FAKTURU')
                            ->content(function ($get) {
                                $total = 0;
    ...
    public function createInvoice(string $status): void
    {
        ...
        $messages = [
            'in_preparation' => [
                'title' => 'Avansna faktura je sačuvana',
                'body' => "Avansna faktura broj {$invoice->invoice_number} je sačuvana kao nacrt. Možete je kasnije izdati."
            ],
            'issued' => [
                'title' => 'Avansna faktura je izdata',
                'body' => "Avansna faktura broj {$invoice->invoice_number} je uspešno izdata i spremna za slanje."
            ],
            'sent' => [
                'title' => 'Avansna faktura je izdata i poslana',
                'body' => "Avansna faktura broj {$invoice->invoice_number} je uspešno izdata i označena kao poslana."
            ]
        ];
        ...
        $this->redirect(AvansnaFakturaResource::getUrl('edit', ['record' => $invoice->id]));
    }
